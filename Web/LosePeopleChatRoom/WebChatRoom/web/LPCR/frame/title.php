...
if(empty($_SESSION['username']) /*|| empty($_SESSION['level']) || empty($_SESSION['username'])*/){
	...
	$mysqli = mysqli_connect('localhost:3306','root','lnmp.org#3954','LPCR');  //登录数据库
	...
	$raw_return = array('userid' => base64_decode($_SESSION['userid']) , 'username' => base64_decode($_SESSION['username']) , 'level' => $_SESSION['level'] , 'permission' => $_SESSION['permission'] , 'exp' => $_SESSION['exp']);
